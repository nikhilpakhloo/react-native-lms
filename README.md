...
- Styling: NativeWind
- Orientation: portrait and landscape supported through `orientation: "default"` and responsive layouts
- Android SDK target: compile SDK 36, target SDK 36
- Android build properties: managed with `expo-build-properties`

Expo SDK 57 documents Node `22.13.x` as the supported Node line. This project enforces Node 22 for app startup because Node 24 can crash Metro/NativeWind during file-map updates.
...
- Profile screen with derived user stats and avatar-oriented native media support.

### Course Ecosystem

- Course data from `randomproducts`.
- Instructor data from `randomusers`.
- FlatList-based course browsing with stable keys and memoized course cards.
...
- WebView-to-native progress and completion messages with JSON validation.
...
`npm run start`, `npm run android`, `npm run ios`, and `npm run web` run `scripts/check-node-version.js` first. If they report Node 24, switch to Node 22 before starting Metro:
...
5. List performance is a first-class requirement, so catalog screens use stable FlatList keys, memoized row components, deduped API data, and cached images.
6. WebView content is local and guarded: metadata is injected from native, messages are parsed as JSON, and unexpected navigation opens externally instead of taking over the player.

## Known Limitations And Review Notes

- Expo SDK 57 should be reviewed with Node 22.13.x. Node 24 is intentionally blocked for app startup because Metro/NativeWind can crash with `addedFiles` file-map errors.
- Deprecated Expo APIs are avoided; for example, Image Picker uses `mediaTypes: "images"` instead of deprecated `ImagePicker.MediaTypeOptions`.
- Local notifications are demonstrated with Expo Notifications. Push notification tokens and server push delivery are out of scope for this Mini LMS.
- Offline lesson support stores durable lesson metadata and local WebView content; it does not download remote videos or large binary assets.
- Android build validation requires a local Android SDK with platform 36 and compatible Build Tools installed.
